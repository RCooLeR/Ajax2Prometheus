...
Ajax2Prometheus can publish Home Assistant MQTT discovery configs and retained device state. This makes every Ajax device from `devices.json` appear in Home Assistant as a separate device with multiple entities.

## Prerequisites

- Home Assistant has the MQTT integration enabled.
- Home Assistant and Ajax2Prometheus can reach the same MQTT broker.
- `data/devices.json` contains your Ajax devices with real `zone`, `name`, `room`, `kind`, and `events` values.

## Enable MQTT

Add MQTT settings to the `ajax2prometheus` service in `docker-compose.yml`.
...
services:
  ajax2prometheus:
    environment:
      ...
      AJAX2PROM_MQTT_USERNAME: "your_mqtt_user"
      AJAX2PROM_MQTT_PASSWORD: "your_mqtt_password"
      AJAX2PROM_MQTT_CLIENT_ID: "ajax2prometheus"
      AJAX2PROM_MQTT_TOPIC_PREFIX: "ajax2prometheus"
      AJAX2PROM_MQTT_DISCOVERY: "true"
      AJAX2PROM_MQTT_DISCOVERY_PREFIX: "homeassistant"
      AJAX2PROM_MQTT_RETAIN: "true"
```

If the broker has no username or password, only the broker URL is required:

```yaml
AJAX2PROM_MQTT_BROKER: "tcp://HOME_ASSISTANT_IP:1883"
```
...
Use the hostname or IP address that is reachable from the Ajax2Prometheus container.
...
```powershell
docker compose logs -f ajax2prometheus
```
...
4. Search for your Ajax device names from `devices.json`.

Each Ajax account appears as one Home Assistant device. Each Ajax zone/device from `devices.json` also appears as one Home Assistant device.
...
```powershell
mosquitto_sub -h HOME_ASSISTANT_IP -u your_mqtt_user -P your_mqtt_password -t "ajax2prometheus/#" -v
```
...
```text
ajax2prometheus/
homeassistant/binary_sensor/ajax2prometheus/
homeassistant/sensor/ajax2prometheus/
```
...
- Check `AJAX2PROM_MQTT_BROKER` is reachable from the Ajax2Prometheus container.
- Check MQTT username and password.
- Check MQTT discovery is enabled in Home Assistant.
- Check `AJAX2PROM_MQTT_DISCOVERY_PREFIX` matches Home Assistant, usually `homeassistant`.
- Subscribe to `homeassistant/#` and confirm discovery configs are published.

Devices exist but entities are unavailable:

- Subscribe to `ajax2prometheus/status`; it should be `online`.
- Subscribe to `ajax2prometheus/#`; retained state JSON should be visible.
- Check Home Assistant and Ajax2Prometheus use the same MQTT broker.

Wrong device names or rooms:

- Edit `data/devices.json`.
- Restart Ajax2Prometheus so discovery configs are republished.
